...
void mpeServerTCP::threadedFunction()
{
	while(isThreadRunning()){
		...
		/*
			...
			if(numConnectedNow != numConnectedClients){
				for(int i = 0; i < numExpectedClients; i++){
					if(connections[i].started && !server.isClientConnected(connections[i].tcpServerIndex)){
						connections[i].started = false;
					}
				}
				printClientStatus();
			}
			numConnectedClients = numConnectedNow;
		 */
